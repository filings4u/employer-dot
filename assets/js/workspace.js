(async()=>{
  ...
  if(d.requires_workspace_selection){const seen=new Set();const rows=(d.workspaces||[]).filter(w=>{const k=String(w.organization_id||w.membership_id||'');if(seen.has(k))return false;seen.add(k);return true});document.getElementById('choices').innerHTML=rows.map(w=>`<a class="card workspace-choice" style="display:block;margin:8px 0" href="/workspace.html?id=${encodeURIComponent(w.membership_id)}"><strong>${w.organization_name||w.plan_name||'DOT Employer'}</strong><span style="display:block;margin-top:4px;color:#66758a;font-size:12px">${w.plan_name||'DOT Employer Plan'}</span></a>`).join('');return}
